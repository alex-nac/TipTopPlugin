...
public class TipTopSettingsForm {
    ...
    public boolean isModified(Settings s) {
        if (myMenus.isSelected() != s.isMenusEnabled()) return true;
        if (myToolbarButtons.isSelected() != s.isToolbarButtonsEnabled()) return true;
        if (myToolWindowButtons.isSelected() != s.isToolWindowButtonsEnabled()) return true;
        if (myAllButtons.isSelected() != s.isAllButtonsEnabled()) return true;
        if (myTextColor.getSelectedColor() != s.getTextColor()) return true;
        if (myBackgroundColor.getSelectedColor() != s.getBackgroundColor()) return true;
        if (myBorderColor.getSelectedColor() != s.getBorderColor()) return true;
        if (!myDisplayTime.getValue().equals(s.getDisplayTime())) return true;
        if (!myAnimationDelay.getValue().equals(s.getFlashAnimationDelay())) return true;
        if (!myProposeToCreateShortcutCount.getValue().equals(s.getProposeToCreateShortcutCount())) return true;
        if (!myPopupTemplate.getText().equals(s.getPopupTemplate())) return true;
        if (myFixedTipPosition.isSelected() != s.isFixedTipPosition()) return true;

        return false;
    }

    public void importFrom(Settings s) {
        myMenus.setSelected(s.isMenusEnabled());
        myToolbarButtons.setSelected(s.isToolbarButtonsEnabled());
        myToolWindowButtons.setSelected(s.isToolWindowButtonsEnabled());
        myAllButtons.setSelected(s.isAllButtonsEnabled());
        myTextColor.setSelectedColor(s.getTextColor());
        myBackgroundColor.setSelectedColor(s.getBackgroundColor());
        myBorderColor.setSelectedColor(s.getBorderColor());
        myDisplayTime.setValue(s.getDisplayTime());
        myAnimationDelay.setValue(s.getFlashAnimationDelay());
        myProposeToCreateShortcutCount.setValue(s.getProposeToCreateShortcutCount());
        myPopupTemplate.setText(s.getPopupTemplate());
        myFixedTipPosition.setSelected(s.isFixedTipPosition());
    }

    public Settings exportDisplayedSettings() {
        Settings s = new Settings();

        s.setMenusEnabled(myMenus.isSelected());
        s.setToolbarButtonsEnabled(myToolbarButtons.isSelected());
        s.setToolWindowButtonsEnabled(myToolWindowButtons.isSelected());
        s.setAllButtonsEnabled(myAllButtons.isSelected());
        s.setTextColor(myTextColor.getSelectedColor());
        s.setBackgroundColor(myBackgroundColor.getSelectedColor());
        s.setBorderColor(myBorderColor.getSelectedColor());
        s.setDisplayTime(new Integer(myDisplayTime.getValue().toString()));
        s.setFlashAnimationDelay(new Integer(myAnimationDelay.getValue().toString()));
        s.setProposeToCreateShortcutCount(new Integer(myProposeToCreateShortcutCount.getValue().toString()));
        s.setPopupTemplate(myPopupTemplate.getText());
        s.setFixedTipPosistion(myFixedTipPosition.isSelected());
